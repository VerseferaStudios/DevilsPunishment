...
namespace BeardedManStudios.Forge.Networking.Unity
{
	public partial class NetworkManager : MonoBehaviour
	{
		...
		public GameObject[] PlayerInformationNetworkObject = null;
		...
		private void CaptureObjects(NetworkObject obj)
		{
			...
			else if (obj is PlayerInformationNetworkObject)
			{
				MainThreadManager.Run(() =>
				{
					NetworkBehavior newObj = null;
					if (!NetworkBehavior.skipAttachIds.TryGetValue(obj.NetworkId, out newObj))
					{
						if (PlayerInformationNetworkObject.Length > 0 && PlayerInformationNetworkObject[obj.CreateCode] != null)
						{
							var go = Instantiate(PlayerInformationNetworkObject[obj.CreateCode]);
							newObj = go.GetComponent<PlayerInformationBehavior>();
						}
					}
		...
		[Obsolete("Use InstantiatePlayerInformation instead, its shorter and easier to type out ;)")]
		public PlayerInformationBehavior InstantiatePlayerInformationNetworkObject(int index = 0, Vector3? position = null, Quaternion? rotation = null, bool sendTransform = true)
		{
			var go = Instantiate(PlayerInformationNetworkObject[index]);
			var netBehavior = go.GetComponent<PlayerInformationBehavior>();
			var obj = netBehavior.CreateNetworkObject(Networker, index);
			go.GetComponent<PlayerInformationBehavior>().networkObject = (PlayerInformationNetworkObject)obj;
		...
		/// <summary>
		/// Instantiate an instance of PlayerInformation
		/// </summary>
		/// <returns>
		/// A local instance of PlayerInformationBehavior
		/// </returns>
		/// <param name="index">The index of the PlayerInformation prefab in the NetworkManager to Instantiate</param>
		/// <param name="position">Optional parameter which defines the position of the created GameObject</param>
		/// <param name="rotation">Optional parameter which defines the rotation of the created GameObject</param>
		/// <param name="sendTransform">Optional Parameter to send transform data to other connected clients on Instantiation</param>
		public PlayerInformationBehavior InstantiatePlayerInformation(int index = 0, Vector3? position = null, Quaternion? rotation = null, bool sendTransform = true)
		{
			var go = Instantiate(PlayerInformationNetworkObject[index]);
			var netBehavior = go.GetComponent<PlayerInformationBehavior>();
