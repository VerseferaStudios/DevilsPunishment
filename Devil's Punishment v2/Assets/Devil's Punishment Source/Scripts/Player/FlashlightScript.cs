...
public class FlashlightScript : MonoBehaviour
{
    ...
    public void ToggleUpgraded()
    {
        ...
        if (glow.enabled && !beam.enabled)
        {
            glow.enabled = false;
            beam.enabled = true;
            FMODUnity.RuntimeManager.PlayOneShot("event:/Player/Items/Flashlight/Flashlight_On_Player", transform.position);
        }
        else if (!glow.enabled && beam.enabled)
        {
            glow.enabled = beam.enabled = false;
            FMODUnity.RuntimeManager.PlayOneShot("event:/Player/Items/Flashlight/Flashlight_Off_Player", transform.position);
        }
        else
        {
            glow.enabled = true;
