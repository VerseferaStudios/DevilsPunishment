...
public class RoomNew1 : MonoBehaviour
{
    ...
    void Update()
    {
        if(k + 1 < spawnPoints.Length && Time.time > nextTime)
        {
            Vector3 targetPos = new Vector3(spawnPoints[k].transform.position.x, 0.5f, spawnPoints[k + 1].transform.position.z);


            spawn(spawnPoints[k].transform.position, targetPos, false); //false
            spawn(targetPos, spawnPoints[k + 1].transform.position, true);
